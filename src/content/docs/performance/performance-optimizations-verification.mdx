...
**Limitations**:
- **Service Worker requires HTTPS** - Will not work in development (localhost exception)
- **Browser Support** - Not supported in IE11 (but app doesn't support IE11 anyway)
- **Cache Versioning** - Manual cache version increment needed for updates (currently `v1`)
...
**Limitations**:
- **HTTP/2 Server Push** - Cloudflare may or may not use Link headers for Server Push (depends on Cloudflare configuration)
- **Cache Invalidation** - Static assets cached for 1 year require cache busting via filename changes (Vite handles this automatically with content hashes)
