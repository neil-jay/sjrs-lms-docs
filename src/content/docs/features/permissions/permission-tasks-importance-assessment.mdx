...
- **15 files still use role checks**: MemberEditModal, MemberList, OrderList, LoanTable, Professor hooks, etc.
...
- **Some use direct `hasPermission`**: Still secure, just different pattern
- **Some have custom logic**: Special cases (owner checks, etc.)
...
- **Permission checks happen at API level**: Service layer validation might be redundant
