...
1. **Added `@deprecated` JSDoc to `allowIf` parameter:**
   -  Warns against using `allowIf: () => true`
 ...
 * 
 *  WARNING: Using `allowIf: () => true` completely bypasses all permission checks and is a security risk.
 * 
 ...
 * 
 *  WARNING: Do NOT use this function to bypass permission checks.
