...
- **Solution:** Verify `https://your-api-domain.com` is accessible

### **Symptom: "Authentication failed"**
- **Check:** Auth cookie present, session expired, account status
- **Solution:** Re-authenticate (cookie-based login) and retry `GET /api/auth/me`
...
curl -i -c cookies.txt -X POST "https://your-api-domain.com/api/auth/force-logout-other-sessions" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "password": "REPLACE_WITH_YOUR_PASSWORD"
...
await unifiedAPIClient.get('/api/auth/me');
```
...
#### **Solutions**
```bash
# Bootstrap CSRF so the XSRF-TOKEN cookie exists
curl -i -b cookies.txt "https://your-api-domain.com/api/auth/csrf"

# For state-changing requests (POST/PUT/PATCH/DELETE), include X-XSRF-TOKEN header
# matching the XSRF-TOKEN cookie value.
```
...
const makeRequest = async (endpoint, options = {}) => {
  ...
  return fetch(`${baseUrl}${endpoint}`, { ...options, credentials: 'include', headers });
};
```
...
curl -b cookies.txt "https://your-api-domain.com/api/auth/validate-session"

# Clean up expired sessions (admin only)
curl -X POST "https://your-api-domain.com/api/auth/sessions/cleanup" \
  -b cookies.txt \
  -H "Content-Type: application/json"
```
...
curl -X POST "https://your-api-domain.com/api/auth/resend-confirmation" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com"
  }'
...
# Link format: https://your-api-domain.com/email-confirmation?token=VERIFICATION_TOKEN
...
curl -X GET "https://your-api-domain.com/api/permissions/check?resource=books&action=create" \
  -b cookies.txt
...
curl -X GET "https://your-api-domain.com/api/books/123" \
  -b cookies.txt
...
curl -X GET "https://your-api-domain.com/api/books" \
  -b cookies.txt

# 2. Search for similar resources
curl -X GET "https://your-api-domain.com/api/books/search?q=title" \
  -b cookies.txt
...
#### **cURL Verbose Mode**
```bash
curl -v -X GET "https://your-api-domain.com/api/books" \
  -b cookies.txt
```

#### **Check Response Headers**
```bash
curl -I -X GET "https://your-api-domain.com/api/books" \
  -b cookies.txt
```
