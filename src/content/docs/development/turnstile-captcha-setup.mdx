...
- Site Key: `REPLACE_WITH_YOUR_TURNSTILE_SITE_KEY` (configured in environment variables)
...
```
- Secret Key: `REPLACE_WITH_YOUR_TURNSTILE_SECRET_KEY` (stored securely)

### Frontend Configuration (Site Key)

**Configured in:**
- Environment variable: `VITE_TURNSTILE_SITE_KEY`
- Site Key: `REPLACE_WITH_YOUR_TURNSTILE_SITE_KEY`
- Added to `env.example` for reference`

**For local development, create a `.env` file:**
```bash
VITE_TURNSTILE_SITE_KEY=REPLACE_WITH_YOUR_TURNSTILE_SITE_KEY
```
...
- Error logged: "TURNSTILE_SECRET_KEY not configured"
