...
## Overview
- Auth is cookie-based. The backend sets the JWT in an HttpOnly cookie; the browser sends it automatically.
- JavaScript cannot read the JWT (by design). Auth state is determined by calling `GET /api/auth/me`.
- `tokenManager` is used for session/UI state (not JWT storage).
- On protected navigation, `AuthGuard` triggers `refreshToken()` when the app is not yet authenticated to hydrate the user.
...
- On every protected navigation:
  1. If `isAuthenticated` is true but user data is not hydrated yet, wait for `userDataLoaded`.
  2. If `isAuthenticated` is false, call `refreshToken()` to hydrate via `GET /api/auth/validate-session` then `GET /api/auth/me`.
  3. If refresh fails with 401/403, `logout()` runs and `AuthGuard` redirects to `/login` via a logout event.
  4. When `userDataLoaded` is true and `user` is present, evaluate role/status gates.
- Status gates:
