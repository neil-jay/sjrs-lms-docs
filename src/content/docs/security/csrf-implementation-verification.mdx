...
1. **CSRF Attacks**: Attacker cannot forge state-changing requests because cross-site pages cannot read the `XSRF-TOKEN` cookie and cannot set a matching `X-XSRF-TOKEN` header (double-submit + Origin/Referer allowlist)
...
5. **Replay Attacks**: Tokens expire after configured duration (default: 30 minutes)
